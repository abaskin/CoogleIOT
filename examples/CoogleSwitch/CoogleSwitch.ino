...
#define SWITCH_BASE_TOPIC_WILDCARD SWITCH_BASE_TOPIC "#"
...
void setup() {
  ...
  iot->enableSerial(SERIAL_BAUD)
      .MQTTSubTopic(
          ...
          [=](const char *topic, const char *payload,
            ...
            iot->logPrintf(DEBUG,
                           "Processing request for switch %d on pin %d\n",
                           switchId, switchPin);
            ...
            iot->MQTTQueueMessage(msg, iot->MQTTQoS(), true, switchState == HIGH ? "1" : "0");
          })
      .initialize();
  ...
  for(int i = 0; i < sizeof(switches) / sizeof(int); i++) {
	  ...
	  iot->MQTTQueueMessage(msg, iot->MQTTQoS(), true, "0");
		...
		if(buttonReadings[i] == HIGH) {
			...
			if((millis() - lastToggleTimes[i]) > DEBOUNCE_TIME) {
				...
				if(digitalRead(switches[i]) == HIGH) {
					...
					if(iot->mqttActive()) {
						iot->MQTTQueueMessage(msg, iot->MQTTQoS(), true, "1");
					}
				}
